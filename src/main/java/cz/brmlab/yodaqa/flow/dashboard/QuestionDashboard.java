...
/**
 * A question tracking device. This singleton class is used to keep track of all questions posed and
 * possibly answered so far, record progress on answering them, and so on. This is not useful for
 * the basic io modalities (like interactive or tsvgs), but gets relevant for the web io where we
 * can query the question answering state asynchronously.
 */
...
public final class QuestionDashboard {
	...
	public void addParseTreeForSentence(String sentence, String parseTree) {
		parseTreeForSentence.put(sentence, parseTree);
	}

	public String getParseTreeForSentence(String sentence) {
		return parseTreeForSentence.get(sentence);
	}
 ...
 /* XXX: This could be also a queue.  But we might want to do
	...
	/**
	 * Get a question to answer by the pipeline. Blocks in case there isn't any right now.
	 */
	...
	public synchronized List<Question> getQuestionsToAnswer() {
		return new ArrayList<>(questionsToAnswer);
	}

	public synchronized List<Question> getQuestionsInProgress() {
		return new ArrayList<>(questionsInProgress);
	}

	public synchronized List<Question> getQuestionsAnswered() {
		return new ArrayList<>(questionsAnswered);
	}
};
