...
/**
 * Create CandidateAnswer for the largest NP covering governor of NSUBJ where
 * the dependent is the question LAT.
 *
 * In human language, the focus of question "What is the critical mass of
 * plutonium?" is "mass"; in passage "A bare critical mass of plutonium at
 * normal density is roughly 10 kilograms.", we find out that NSUBJ dependent is
 * "mass", so we look at the governor ("kilograms") and take the largest
 * covering NP ("roughly 10 kilograms").
 *
 * Originally, we thought of this as focus matching, but (sp=0) LAT is better as
 * we do desirable transformations like who->person or hot->temperature when
 * generating these, and they *are* focus based.
 *
 * Of course this is a special case of CanByNPSurprise but we should be subduing
 * those.
 */
@SofaCapability(
            ...
            if (subjlemma.equals(latlemma)) {
                ...
                if (base instanceof Token) {
                    ...
                    if (pos.matches("^v.*") || pos.matches("^V.*")) {
                        for (PREDC predc : JCasUtil.selectCovered(PREDC.class, p)) {
                            base = TreeUtil.widestCoveringAlpinoNP(predc.getDependent());
                            if (base == null) {
                                base = predc.getDependent();
                            }
                        }
                        if (base == null) {
