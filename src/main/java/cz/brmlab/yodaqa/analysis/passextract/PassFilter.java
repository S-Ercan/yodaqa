 ...
 * them over to the Passages view.
 *
 * Prospectively, we might want to keep some surrounding sentences, though.
 */
@SofaCapability(
        inputSofas = {"Passages"},
...
public class PassFilter extends JCasAnnotator_ImplBase {
    ...
    /**
     * Number of passages to pick for detailed analysis. Keep in sync with the
    ...
    public void process(JCas jcas) throws AnalysisEngineProcessException {
        ...
        PickedPassageInfo ppInfo;
        ppInfo = new PickedPassageInfo(pickedPassagesView);
        ppInfo.setNumPassages(ppInfo.getNumPassages() + Math.min(i, numPicked));
        ppInfo.addToIndexes(pickedPassagesView);
    }
}
